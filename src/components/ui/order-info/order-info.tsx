...
export const OrderInfoUI: FC<OrderInfoUIProps> = memo(
  ({ orderInfo, showNumberHeading }) => (
    <div className={styles.wrap}>
      {showNumberHeading && (
        <h3 className={`text text_type_digits-default pt-10 ${styles.number}`}>
          #{String(orderInfo.number).padStart(6, '0')}
        </h3>
      )}
      <h3 className={`text text_type_main-large pb-3 pt-6 ${styles.header}`}>
        {orderInfo.name}
      </h3>
